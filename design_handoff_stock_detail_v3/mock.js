...
(function () {
  ...
  // ── Belgian capital gains tax (meerwaardebelasting, wet van 3 april 2026) ──
  // foto = waarde op 31/12/2025 (peildatum / step-up basis)
  const FOTO_F = [0.86, 0.94, 1.05, 0.78, 0.98, 0.9];
  POSITIONS.forEach((p, i) => {
    p.fotoValueEur = Math.round(p.value * FOTO_F[i % FOTO_F.length]);
    p.taxGainEur = p.value - p.fotoValueEur; // unrealized taxable gain vs foto
  });
  const TAX = {
    year: 2026,
    rate: 0.1,
    exemptionBase: 10000, // per person, indexed; 20000 for couples (gemeenschap van goederen)
    realized: [
      { date: '2026-02-11', ticker: 'SXRT', shares: 12, proceeds: 6480, basis: 3360, basisType: 'foto', gain: 3120, withheld: 0, note: 'transition — self-declare' },
      { date: '2026-05-06', ticker: 'EUDF', shares: 40, proceeds: 2160, basis: 3000, basisType: 'aankoop', gain: -840, withheld: 0, note: 'actual cost basis (higher than foto)' },
      { date: '2026-06-24', ticker: 'ASTS', shares: 15, proceeds: 6820, basis: 4870, basisType: 'foto', gain: 1950, withheld: 195, note: 'broker withheld 10%' },
    ],
  };
  ...
  window.PT_MOCK = {
    COLORS, POSITIONS, WATCHLIST, TAX, chartData, intraday, portfolioIntraday, portfolioIntradayFlaky,
    transactions,
    totals: {
