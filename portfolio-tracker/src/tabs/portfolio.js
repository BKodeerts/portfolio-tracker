...
function buildRenderContext(filtered) {
  ...
  const periodPct = (
    Number.parseFloat(latest.pctReturn) - Number.parseFloat(first.pctReturn)
  ).toFixed(1);
  ...
  const visibleTickers = state.showClosed
    ? Object.keys(state.TICKER_META)
    : state.CURRENT_TICKERS;
  const closedTrack = state.showClosed ? "on" : "";
  const closedToggleHtml =
...
function yBoundsRange(view, datasets, tickerVals, visibleTickers) {
  ...
  if (view === "pl") {
    const prevTotal = visibleTickers.reduce(
      (s, t) => s + (tickerVals[t]?.prevValueEur || 0),
      0,
    );
    return { floorMin: -prevTotal * 0.02, ceilMax: prevTotal * 0.02 };
  }
  const ref = datasets.find((d) => d.label === "Vorige slotkoers")?.data?.[0]
    ?.y;
...
function buildYBounds(datasets, floorMin, ceilMax) {
  ...
  return {
    min: Math.min(dMin - pad, floorMin),
    max: Math.max(dMax + pad, ceilMax),
  };
}
...
function makeSegment(color, hasEU, hasUS) {
  return {
    borderColor: (ctx) => {
      const mins =
        new Date(ctx.p0.parsed.x).getHours() * 60 +
        new Date(ctx.p0.parsed.x).getMinutes();
      const active =
        (hasEU && mins >= 540 && mins < 1050) ||
        (hasUS && mins >= 930 && mins < 1320);
  ...
  if (view === "total") {
    const totals = timestamps.map((_, i) =>
      visibleTickers.reduce(
        (sum, t) => sum + (tickerVals[t]?.values[i] || 0),
        0,
      ),
    );
    const prevCloseTotal = visibleTickers.reduce(
      (sum, t) => sum + (tickerVals[t]?.prevValueEur || 0),
      0,
    );
    ...
    return [
      ...
      },
      {
        ...
        data: [
          { x: dayStart, y: prevCloseTotal },
          { x: dayEnd, y: prevCloseTotal },
        ],
        borderColor: chartTheme().costLine,
...
function buildHistoricalDatasets(visibleTickers) {
  ...
  if (view === "total") {
    return [
      ...
      {
        label: "Kostprijs",
        data: filtered.map((d) => d.totalCost),
        borderColor: chartTheme().costLine,
        backgroundColor: chartTheme().costFill,
        fill: true,
        borderWidth: 1,
        borderDash: [4, 4],
        pointRadius: 0,
  ...
  if (view === "individual") {
    return [...visibleTickers].reverse().map((ticker) => ({
      label: ticker,
      data: filtered.map((d) => d[ticker] || 0),
      borderColor: getColor(ticker),
      backgroundColor: getColor(ticker) + "28",
      fill: true,
      borderWidth: 1.5,
      pointRadius: 0,
      tension: 0,
  ...
  if (view === "pct") {
    return visibleTickers.map((ticker) => ({
      ...
      data: filtered.map((d) =>
        d[`${ticker}_pct`] == null
          ? null
          : Number.parseFloat(d[`${ticker}_pct`]),
      ),
      borderColor: getColor(ticker),
      fill: false,
      borderWidth: 2,
  ...
  return [...visibleTickers].reverse().map((ticker) => ({
    label: ticker,
    data: filtered.map((d) =>
      d[ticker] != null && d[`${ticker}_cost`] != null
        ? d[ticker] - d[`${ticker}_cost`]
        : null,
    ),
    borderColor: getColor(ticker),
    backgroundColor: getColor(ticker) + "28",
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          ...
          borderWidth: 1,
          titleColor: chartTheme().titleColor,
          bodyColor: chartTheme().bodyColor,
          ...
          callbacks: {
            title: (items) =>
              useIntraday
                ? new Date(items[0].parsed.x).toLocaleTimeString("nl-BE", {
                    hour: "2-digit",
                    minute: "2-digit",
                  })
                : new Date(items[0].label).toLocaleDateString("nl-BE", {
                    day: "numeric",
                    month: "short",
                    year: "numeric",
                  }),
    ...
    loadIntradayData(false, () => {
      ...
      if (state.currentPeriod === "1d" && state.currentTab === "portefeuille") {
        if (state.chartInstances.main) {
          ...
          delete state.chartInstances.main;
        }
        renderPortfolioChart(visibleTickers);
        renderLegend(visibleTickers);
        updatePeriodChange();
