...
function isOpenNow(tz, openH, openM, closeH, closeM) {
...
function isExchangeOpen(yahooSymbol) {
...
function normalizeMarketState(yahooSymbol, rawState) {
  if (rawState === "POST" && EU_EXCHANGE_RE.test(yahooSymbol)) return "CLOSED";
  return rawState;
}
function sparklineSVG(points, prevClose, tradingMins, muted = false) {
  ...
  const fillPath = `M${xs[0].toFixed(1)},${ys[0].toFixed(1)} ` + xs.slice(1).map((x, i) => `L${x.toFixed(1)},${ys[i + 1].toFixed(1)}`).join(" ") + ` L${xs[xs.length - 1].toFixed(1)},${H} L${xs[0].toFixed(1)},${H} Z`;
